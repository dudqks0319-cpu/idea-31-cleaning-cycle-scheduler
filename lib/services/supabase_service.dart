...
class SupabaseService {
  ...
  Future<List<EnergyTask>> fetchTasks(String userId) async {
    if (useSupabase) {
      final rows = await Supabase.instance.client
          .from('cleaning_cycle_scheduler_items')
  ...
  Future<void> saveTask({required String userId, required EnergyTask task}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('cleaning_cycle_scheduler_items').upsert(task.toInsertMap(userId), onConflict: 'id');
      return;
    }
  ...
  Future<void> deleteTask({required String userId, required String taskId}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('cleaning_cycle_scheduler_items').delete().eq('id', taskId).eq('user_id', userId);
      return;
    }
